...
function renderCommentsList(comments_list, range_random) {
  bullet_allnum.innerHTML = `Already have${comments_list.length}comments`;
  ...
  if (form) { // 确保找到了表单
    form.addEventListener('submit', async function (event) {
      ...
      try {
        ...
        if (!response.ok) {
          document.querySelector('.info_tip').innerHTML = `The request failed, please check the network connection or contact the administrator`;
          ...
          document.querySelector('.info_tip').innerHTML = `Submit Successful: ${responseMessage}`;
          document.querySelector('.info_tip').classList.add('info_tip_animate');
          let clearTime = setTimeout(() => {
            document.querySelector('.info_tip').classList.remove('info_tip_animate');
          }, 2000);
